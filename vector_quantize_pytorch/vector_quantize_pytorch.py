...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        commitment_weight = 1.,
        vq_objective_weight = 0.25, # jsbae
        ...
        self.vq_objective_weight = vq_objective_weight # jsbae
        ...
        if self.training:
            if self.commitment_weight > 0:
                if self.commitment_use_cross_entropy_loss:
                    ...
                    if exists(mask):
                        ...
                        if not self.learnable_codebook: # jsbae
                            commit_loss = F.mse_loss(commit_quantize, x, reduction = 'none') # jsbae
                        else: # jsbae
                            commit_loss = F.mse_loss(commit_quantize.detach(), x, reduction = 'none') # jsbae
                        ...
                        commit_loss = commit_loss[loss_mask].sum(dim=-1).mean() # jsbae
                    else:
                        if not self.learnable_codebook: # jsbae
                            # commit_loss = F.mse_loss(commit_quantize, x)
                            commit_loss = F.mse_loss(commit_quantize, x, reduction = 'none').sum(dim=-1).mean() # jsbae
                        else: # jsbae
                            # commit_loss = F.mse_loss(commit_quantize.detach(), x) # jsbae
                            commit_loss = F.mse_loss(commit_quantize.detach(), x, reduction = 'none').sum(dim=-1).mean() # jsbae

                loss = loss + commit_loss * self.commitment_weight

                if self.learnable_codebook: # jsbae
                    # vq_objective = F.mse_loss(commit_quantize, x.detach()) # jsbae
                    vq_objective = F.mse_loss(commit_quantize, x.detach(), reduction = 'none').sum(dim=-1).mean() # jsbae
                    loss = loss + vq_objective * self.vq_objective_weight # jsbae
            if self.has_codebook_orthogonal_loss:
                ...
                if exists(self.orthogonal_reg_max_codes) and num_codes > self.orthogonal_reg_max_codes:
                    rand_ids = torch.randperm(num_codes)[:self.orthogonal_reg_max_codes]
                    rand_ids = rand_ids.to(device)
