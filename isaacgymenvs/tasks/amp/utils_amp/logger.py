...
def set_file_handler(path=None, prefix="", time_str=""):
    ...
    if time_str == "":
        file_name = prefix + datetime.now(timezone(timedelta(hours=9))).strftime("_%Y-%b-%d-%H-%M-%S") + ".log"
    else:
    ...
    file_handler = logging.FileHandler(filename=os.path.join(path, "logger"), encoding="utf-8", mode="w")
